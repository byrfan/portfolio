...
typedef struct connection {
    ...
    response_node_t *response_queue_head;
    response_node_t *response_queue_tail;
...
void free_response_queue(connection_t *conn) {
    ...
    response_node_t *cur = conn->response_queue_head;
    while (cur) {
        response_node_t *next = cur->next;
        free(cur);
        cur = next;
    }

    conn->response_queue_head = conn->response_queue_tail = conn->current_response = NULL;
}
...
void queue_response(connection_t *conn, int status, const char *type, const char *body) {
    ...
    if (len <= 0 || len >= sizeof(response)) return;
    ...
    if (!conn->response_queue_head) {
        conn->response_queue_head = conn->response_queue_tail = node;
    } else {
        conn->response_queue_tail->next = node;
        conn->response_queue_tail = node;
    }
}
...
void handle_ssl_accept(connection_t *conn, int epoll_fd) {
    while (1) {
        ...
        if (err == SSL_ERROR_WANT_READ || err == SSL_ERROR_WANT_WRITE) {
            struct epoll_event ev;
            ev.events = (err == SSL_ERROR_WANT_READ ? EPOLLIN : EPOLLOUT) | EPOLLRDHUP;
            ev.data.ptr = conn;
            epoll_ctl(epoll_fd, EPOLL_CTL_MOD, conn->fd, &ev);
            return;
        }
...
void handle_read(connection_t *conn, int epoll_fd) {
    ...
    while ((bytes = SSL_read(conn->ssl, buffer, sizeof(buffer))) > 0) {
        ...
        char *end;
        while ((end = strstr(current, "\r\n\r\n")) != NULL) {
            char method[16], uri[256], version[16];
            sscanf(current, "%15s %255s %15s", method, uri, version);
        ...
        int remaining = conn->bytes_read - (current - conn->buffer);
        if (remaining > 0)
            memmove(conn->buffer, current, remaining);
    ...
    if (conn->current_response || conn->response_queue_head) {
        struct epoll_event ev;
...
void handle_write(connection_t *conn, int epoll_fd) {
    ...
    while (1) {
        if (!conn->current_response && conn->response_queue_head) {
            conn->current_response = conn->response_queue_head;
        ...
        if (bytes > 0) {
            ...
            int err = SSL_get_error(conn->ssl, bytes);
            if (err == SSL_ERROR_WANT_WRITE) return;
            ERR_print_errors_fp(stderr);
            close_connection(conn, epoll_fd);
            return;
        }
        ...
        for (int i = 0; i < nfds; i++) {
            if (events[i].data.fd == server_fd) {
                ...
                if (!SSL_is_init_finished(conn->ssl)) {
                    ...
                    if (events[i].events & EPOLLIN) handle_read(conn, epoll_fd);
                    if (events[i].events & EPOLLOUT) handle_write(conn, epoll_fd);
                }
            }
        }
    }
